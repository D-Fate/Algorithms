...
// осуществляющую поразрядную сортировку элементов динамического списка А, 
// состоящего из 50 элементов. Элементами являются даты текущего столетия,
// представленные в виде одной строки, в которой день и год
// записаны цифрами (день - двумя, год - четырьмя), а месяц - словом. 
// Элементы можно вводить с клавиатуры, читать из файла или
...
int getDays(int month) {
    // количество дней в месяце без учета високосности
...
void List::radixSort() {
    for (int i = 0; i < 3; i++) {
        int length = MAX_DATE_VALUES[i] - MIN_DATE_VALUES[i] + 1;
        List digits[length];
        while (tail_ != nullptr) {
